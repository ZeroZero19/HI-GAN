...
parser.add_argument('--data-root', default='testsets/demo_cell', type=str, help='dir to dataset')
...
test_batch_size = args_test.batch_size
test_seed = 13
cmap = 'inferno'
...
out_dir = args_test.out_dir
mkdirs(out_dir)


if args_test.net == 'N2N':
    model = N2N(1, 1).to(device)
    model.load_state_dict(torch.load(args_test.model))
elif args_test.net == 'DnCNN':
    ...
    model.load_state_dict(torch.load(args_test.model))
elif args_test.net == 'UNet_ND':
    model = UNet_ND_cell().to(device)
    model = nn.DataParallel(model, list(range(args_test.cuda)))
    model.load_state_dict(torch.load(args_test.model))
elif args_test.net == 'UNet_D':
    model = UNet_D_cell().to(device)
    model = nn.DataParallel(model, list(range(args_test.cuda)))
    model.load_state_dict(torch.load(args_test.model))
elif args_test.net == 'HI_GAN':
    model0 = UNet_ND_cell().to(device)
    model1 = UNet_D_cell().to(device)
    model0 = nn.DataParallel(model0, list(range(args_test.cuda)))
    model1 = nn.DataParallel(model1, list(range(args_test.cuda)))
...
for noise_level in noise_levels:
    for image_type in image_types:
        ...
        if image_type == 'FMD_test_mix':
            ...
            n_plots = 12
            test_loader = load_denoising_test_mix_flyv2(args_test.data_root,
                                                  batch_size=test_batch_size, noise_levels=[noise_level],
                                                  transform=four_crop, target_transform=four_crop,
                                                  patch_size=256)
        # four crop
        ...
        for batch_idx, (noisy, clean, name) in enumerate(test_loader):
            ...
            if image_type == 'FMD_test_mix':
                ...
                if 'Avg' in name[0]:
                    img_idx = int(name[0].split('Avg')[1][:-4])
                elif 'raw' in name[0]:
                    img_idx = int(name[0].split('raw')[1][:-4])
            if img_idx<5:
                fixed_denoised_stitched = stitch_pathes(to_numpy(denoised))
                out[test_case_dir + name[0].replace('.png', '_dn.png')] = convert(fixed_denoised_stitched)
                cv2.imwrite(test_case_dir + name[0].replace('.png', '_dn.png'), convert(fixed_denoised_stitched))

                if args_test.save_noi_clean:
                    fixed_noisy_stitched = stitch_pathes(to_numpy(noisy))
                    out[test_case_dir + name[0].replace('.png', '_noi.png')] = convert(fixed_noisy_stitched)
                    cv2.imwrite(test_case_dir + name[0].replace('.png', '_noi.png'), convert(fixed_noisy_stitched))

                    fixed_clean_stitched = stitch_pathes(to_numpy(clean))
                    out[test_case_dir + name[0].replace('.png', '_clean.png')] =  convert(fixed_clean_stitched)
                    cv2.imwrite(test_case_dir + name[0].replace('.png', '_clean.png'), convert(fixed_clean_stitched))
        ...
        with open(out_dir + "/results_{}_{}.txt".format('cpu' if args_test.no_cuda else 'gpu',image_type), 'w') as args_file:
