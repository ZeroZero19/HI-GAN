...
def AddNoiseMosai(x, CRF_para, iCRF_para, I, B, Iinv, Binv, sigma_s, sigma_c, crf_index, pattern, opt = 1):
    ...
    return result, noise_s, noise_c

def AddRealNoise(image, CRF_para, iCRF_para, I_gl, B_gl, I_inv_gl, B_inv_gl):
    sigma_s = np.random.uniform(0.0, 0.16, (3,)) # 0.16
    sigma_c = np.random.uniform(0.0, 0.06, (3,)) # 0.06
...
def AddRealNoiseGP(image, CRF_para, iCRF_para, I_gl, B_gl, I_inv_gl, B_inv_gl):
    ...
    CRF_index = np.random.choice(201)
    pattern = np.random.choice(4) + 1
    noise_img, noise_s, noise_c = AddNoiseMosai(image, CRF_para, iCRF_para, I_gl, B_gl, I_inv_gl, B_inv_gl, sigma_s, sigma_c, CRF_index, pattern, 0)
