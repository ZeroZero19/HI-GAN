...
def normalize(data):
    b"""Normalizes a unit8 image to a float32 image in the range [0, 1]
...
def variable_to_cv2_image(varim):
    b"""Converts a torch.autograd.Variable to an OpenCV image
...
def batch_psnr_ssim(img, imclean, data_range=1., batch_ssim=True):
    ...
    ssims = []
    for i in range(img_cpu.shape[0]):
        # psnr_dn += compare_psnr(imgclean[i, :, :, :], img_cpu[i, :, :, :], data_range=data_range)
        im1 = np.array(img_cpu[i, :, :, :].transpose((1, 2, 0)) * 255.0, dtype='uint8')
        im2 = np.array(imgclean[i, :, :, :].transpose((1, 2, 0)) * 255.0, dtype='uint8')
        im1_y = cv2.cvtColor(im1, cv2.COLOR_BGR2YCR_CB)[:, :, 0]
        im2_y = cv2.cvtColor(im2, cv2.COLOR_BGR2YCR_CB)[:, :, 0]
...
def data_augmentation(image, mode):
    b"""Performs dat augmentation of the x image
...
def init_logger_data(data_dir):
    b"""Initializes a logging.Logger in order to log the data
