...
def find_lr(net, trn_loader, optimizer, loss_fn, init_value=1e-8, final_value=10., beta=0.98, device='cuda:1'):
    ...
    for (noisy, clean, _) in trn_loader:
        ...
        #As before, get the pixel_loss for this mini-batch of inputs/outputs
        optimizer.zero_grad()
        output = net(noisy)
        loss = loss_fn(output, clean)
        # pixel_loss = criterion(outputs, labels)
        #Compute the smoothed pixel_loss
        avg_loss = beta * avg_loss + (1-beta) *loss.item()
        smoothed_loss = avg_loss / (1 - beta**batch_num)
        #Stop if the pixel_loss is exploding
        if batch_num > 1 and smoothed_loss > 4 * best_loss:
            return log_lrs, losses
        #Record the best pixel_loss
