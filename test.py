...
from _utils.utils import normalize
from torchvision.utils import save_image, make_grid
from collections import OrderedDict

parser = argparse.ArgumentParser()
parser.add_argument('--nGPU', type=int, default=1, help='number of GPUs to use')
parser.add_argument('--inp', type=str, default='testsets/DND_patches', help='input folder')
parser.add_argument('--out', type=str, default='results', help='output folder')
parser.add_argument('--JPEG', action='store_true', help="for JPEG images (Nam_patches)")
...
if (device.type == 'cuda') and (ngpu >= 1):
    ...
    try:
        boostnet.load_state_dict(
            torch.load(Boostnet_path))
    except:
        # original saved file with DataParallel
        state_dict = torch.load(Boostnet_path, map_location=lambda storage, loc: storage)
        # create new OrderedDict that does not contain `module.`
        from collections import OrderedDict
        new_state_dict = OrderedDict()
        for k, v in state_dict.items():
            new_state_dict[k[7:]] = v  # remove `module.`
        boostnet.load_state_dict(new_state_dict, strict=False)
...
for tp in types:
    files.extend(glob.glob(os.path.join(testset, tp)))
files.sort()
